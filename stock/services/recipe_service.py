...
Handles: Recipe, RecipeIngredient, RecipeIngredientSubstitute, RecipeByProduct, RecipeStep
"""
...
class RecipeService(BaseService):
    """Manage recipes with full versioning support"""
    ...
    def get_active_for_item(cls, output_item_id: int) -> Optional[Recipe]:
        """Get the active version of recipe for an item"""
    ...
    def update(cls, recipe_id: int, **kwargs) -> Dict[str, Any]:
        """Update recipe (creates new version if significant changes)"""
        recipe = cls.get_by_id(recipe_id)
        if not recipe:
            raise NotFoundError("Recipe", recipe_id)
        
        # Fields that can be updated without versioning
        ...
        # Check if only minor fields are being updated
    ...
    def create_new_version(cls, recipe_id: int, **kwargs) -> Dict[str, Any]:
        ...
        # Find root recipe for versioning
        root = recipe
        while root.parent_recipe:
            root = root.parent_recipe
        ...
        new_recipe = cls.model.objects.create(
            ...
            code=recipe.code,  # Same code, different version
            ...
            is_active_version=False,  # Not active until approved
            parent_recipe=root,
    ...
    def approve(cls, recipe_id: int, approved_by_id: int) -> Dict[str, Any]:
        """Approve recipe and make it the active version"""
        recipe = cls.get_by_id(recipe_id)
        if not recipe:
            raise NotFoundError("Recipe", recipe_id)
    ...
    def scale_recipe(cls, recipe_id: int, 
        ...
        """Scale recipe to target quantity or multiplier"""
        recipe = cls.get_by_id(recipe_id)
        if not recipe:
            raise NotFoundError("Recipe", recipe_id)
...
class RecipeByProductService(BaseService):
    """Manage recipe by-products and waste"""
