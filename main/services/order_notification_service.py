...
class TelegramAPI:
    ...
    def _request(cls, method: str, data: dict) -> Optional[dict]:
        try:
            response = requests.post(f"{cls.BASE_URL}/{method}", json=data, timeout=10)
    ...
    def send_message(cls, chat_id: int, text: str, parse_mode: str = "HTML") -> Optional[int]:
        result = cls._request("sendMessage", {"chat_id": chat_id, "text": text, "parse_mode": parse_mode})
    ...
    def send_sticker(cls, chat_id: int, sticker_id: str) -> Optional[int]:
        if not sticker_id:
            return None
        result = cls._request("sendSticker", {"chat_id": chat_id, "sticker": sticker_id})
        return result.get('message_id') if result else None

    @classmethod
    def edit_message(cls, chat_id: int, message_id: int, text: str, parse_mode: str = "HTML") -> bool:
        result = cls._request("editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": text, "parse_mode": parse_mode})
        return result is not None

    @classmethod
    def pin_message(cls, chat_id: int, message_id: int, disable_notification: bool = False) -> bool:
        result = cls._request("pinChatMessage", {"chat_id": chat_id, "message_id": message_id, "disable_notification": disable_notification})
        return result is not None

    @classmethod
    def unpin_message(cls, chat_id: int, message_id: int) -> bool:
        result = cls._request("unpinChatMessage", {"chat_id": chat_id, "message_id": message_id})
        return result is not None

    @classmethod
    def delete_message(cls, chat_id: int, message_id: int) -> bool:
        result = cls._request("deleteMessage", {"chat_id": chat_id, "message_id": message_id})
        return result is not None
...
"""
    ...
    def process_pending(self) -> tuple:
        if not TelegramAPI.is_online():
            return 0, 0
    ...
    def _format_order_type(self, order_type: str) -> str:
        types = {'HALL': 'Zalda', 'DELIVERY': 'Yetkazib berish', 'PICKUP': 'Olib ketish'}
    ...
    def _send_new_order(self, order_data: dict) -> dict:
        ...
        logger.info(f"Sending order notification to chat_ids: {chat_ids}")
        if not chat_ids:
        ...
        for chat_id in chat_ids:
            try:
                ...
                if message_id:
                    ...
                    success_count += 1
                    logger.info(f"Order notification sent to {chat_id}")
            except Exception as e:
                logger.error(f"Failed to send to {chat_id}: {e}")
        if chat_message_map:
            OrderMessageStorage.save_message_ids(order_data['id'], chat_message_map)
        return {'success': success_count > 0, 'message': f'Sent to {success_count}/{len(chat_ids)} chats', 'message_ids': chat_message_map}
    ...
    def _update_order_message(self, order_data: dict) -> dict:
        ...
        for chat_id, message_id in message_ids.items():
            if TelegramAPI.edit_message(chat_id, message_id, message):
                success_count += 1
        return {'success': success_count > 0, 'message': f'Updated {success_count}/{len(message_ids)} messages'}
    ...
    def _send_order_ready(self, order_data: dict) -> dict:
        ...
        return {'success': success_count > 0, 'message': f'Ready notification sent to {success_count}/{len(chat_ids)} chats'}
    ...
    def _send_order_cancelled(self, order_data: dict) -> dict:
        ...
        OrderMessageStorage.remove_order(order_data['id'])
        return {'success': success_count > 0, 'message': f'Cancelled notification sent to {success_count}/{len(chat_ids)} chats'}
